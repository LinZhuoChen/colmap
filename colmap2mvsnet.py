...
if __name__ == '__main__':
    ...
    max_image = max(images_keys) 
    # num_images = [i - 1 for i in num_images]
    ...
    for key in images_keys:
        ...
        for p3d_id in images[key].point3D_ids:
            if p3d_id == -1:
                continue
            transformed = np.matmul(extrinsic[key], [points3d[p3d_id].xyz[0], points3d[p3d_id].xyz[1], points3d[p3d_id].xyz[2], 1])
    ...
    # view selection
    # score = np.zeros((max_image, max_image))
    from collections import defaultdict
    score = defaultdict(list)
    queue = []
    for i in images_keys:
        # for j in range(key + 1, max_image):
        for j in images_keys:
            if j >= i+1:
                queue.append((i, j))
                
    def calc_score(inputs):
        key1, key2 = inputs
        id_i = images[key1].point3D_ids
        id_j = images[key2].point3D_ids
        id_intersect = [it for it in id_i if it in id_j]
        cam_center_i = -np.matmul(extrinsic[key1][:3, :3].transpose(), extrinsic[key1][:3, 3:4])[:, 0]
        cam_center_j = -np.matmul(extrinsic[key2][:3, :3].transpose(), extrinsic[key2][:3, 3:4])[:, 0]
        score = 0
    ...
    for key in images_keys:
        ...
        sorted_data = sorted(score[key], key=lambda x: x[1], reverse=True)
        view_sel[key] = [(k,s) for (k, s) in sorted_data[:10]]
    # print('view_sel[0]\n', view_sel[0], end='\n\n')
    ...
    for key in images_keys:
        with open(os.path.join(cam_dir, '%08d_cam.txt' % (key)), 'w') as f:
            ...
            for j in range(3):
                ...
                f.write('\n')
            f.write('\n%f %f %f %f\n' % (depth_ranges[key][0], depth_ranges[key][1], depth_ranges[key][2], depth_ranges[key][3]))
    with open(os.path.join(args.dense_folder, 'pair.txt'), 'w') as f:
        f.write('%d\n' % len(images))
        for key, sorted_score in view_sel.items():
            f.write('%d\n%d ' % (key, len(sorted_score)))
            for image_id, s in sorted_score:
                f.write('%d %f ' % (image_id, s))
            f.write('\n')
            
    # for i in range(num_images):
    #     shutil.copyfile(os.path.join(image_dir, images[i+1].name), os.path.join(renamed_dir, '%08d.png' % i))